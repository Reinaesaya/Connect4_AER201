...
void line_follow_intersection(Wheels& wheels, int& num_intersections)
{
  ...
  while ((!intersection_hit))
  {
    ...
    else { wheels.Forward_straight(LINE_FOLLOW_SPEED); }
  }
  num_intersections++;
}
...
void get_to_hopper_pos(Wheels& wheels, Hopper& hopper, Ultrasonic& front_US, Ultrasonic& right_US, Ultrasonic& left_US, float& angle)
{
  ...
  {
    Serial.println("Left Board Hopper");
    delay(10);
    ...
    else if (hopper.corner == NOT_CORNER_HOP)
    {
      ...
      {
        wheels.Pivot_L(90);
        Serial.println(hopper.dist_to_wall);
  ...
  {
    Serial.println("Right Board Hopper");
    my_delay(10);
...
void backward_align(Wheels& wheels)
{
  ...
  int B_R = analogRead(BACK_RIGHT);
  int B_L = analogRead(BACK_LEFT);
  while ((B_R < (MAX_WHITE_B_R + B_W_THRESH/2)) || (B_L < (MAX_WHITE_B_L + B_W_THRESH/2)))
  {
    B_R = analogRead(BACK_RIGHT);
    B_L = analogRead(BACK_LEFT);
    wheels.Backward_straight(85);
...
void forward_start(int left_up, Wheels& wheels)
{
  wheels.encoder.reset();
  int B_R = analogRead(BACK_RIGHT);
  int B_L = analogRead(BACK_LEFT);
  while ((B_R < (MAX_WHITE_B_R + B_W_THRESH)) && (B_L < (MAX_WHITE_B_L + B_W_THRESH)))
  {
    B_R = analogRead(BACK_RIGHT);
    B_L = analogRead(BACK_LEFT);
    wheels.Forward_straight(75);
  }
  wheels.Forward(75,10900);
  B_R = analogRead(BACK_RIGHT);
  B_L = analogRead(BACK_LEFT);
  while ((B_R < (MAX_WHITE_B_R + B_W_THRESH)) && (B_L < (MAX_WHITE_B_L + B_W_THRESH)))
  {
  ...
  wheels.encoder.reset();
  my_delay(250);
  if (left_up) grid_follow(wheels, LEFT_TURN, 0, 2);
  else grid_follow(wheels, RIGHT_TURN, 0, 2);
}
