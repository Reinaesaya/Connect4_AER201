...
void i_hopper_navigation(Wheels& MyWheel, byte mode, byte *num_inter, byte *num_turn, byte x_val, byte y_val,int angle){
  ...
  *num_turn = 0;
  if (mode % 2 == 0)
  {
    if (angle > 180)  MyWheel.Pivot_L(angle-180);
    else if(angle != 0) MyWheel.Pivot_R(180-angle);
  }
  else if(mode %2 != 0)
  {
    if(angle >180)  MyWheel.Pivot_L(360-angle);
    else if(angle != 0) MyWheel.Pivot_R(angle);
  }
  return;
}
