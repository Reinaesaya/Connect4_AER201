...
// Wall follow until front sensor hits certain distance
{
  ...
  while (check_array(front_US.array, stop_dist) != 1)
  {
    ...
    //Serial.println(side_US.array[ARRAY_LEN - 1]);
    delay(10);
...
// Navigate to corner hopper and position for ball grabbing (wall following)
{
  forward_to_dist(hopper.dist_to_wall-TURN_DIFFERENCE, wheels, front_US, 100);
  if (hopper.side == LEFT_BOARD)
  {
    wheels.Pivot_L(90);
    forward_to_dist_wall(RIGHT_BOARD, hopper.dist_to_wall-TURN_DIFFERENCE, GAMEBOARD_WALL_FOLLOW_DIST, wheels, front_US, right_US, WALL_FOLLOW_SPEED);
    wheels.Pivot_L(90);
    forward_to_dist_wall(RIGHT_BOARD, hopper.dist_to_wall-PIVOT_TO_FRONT_US, hopper.dist_to_wall-PIVOT_TO_SIDE_US, wheels, front_US, right_US, WALL_FOLLOW_SPEED);
    wheels.Pivot_R(45);
  }
  else if (hopper.side == RIGHT_BOARD)
  {
    wheels.Pivot_R(90);
    forward_to_dist_wall(LEFT_BOARD, hopper.dist_to_wall-TURN_DIFFERENCE, GAMEBOARD_WALL_FOLLOW_DIST, wheels, front_US, left_US, WALL_FOLLOW_SPEED);
    wheels.Pivot_R(90);
    forward_to_dist_wall(LEFT_BOARD, hopper.dist_to_wall-PIVOT_TO_FRONT_US, hopper.dist_to_wall-PIVOT_TO_SIDE_US, wheels, front_US, left_US, WALL_FOLLOW_SPEED);
    wheels.Pivot_L(45);
  }
  angle = hopper.angle;
}
