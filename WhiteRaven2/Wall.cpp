...
void wall_straight_adjust(Ultrasonic& side_US, int SPD, Wheels& wheels)
...
void wall_dist_adjust(Ultrasonic& side_US, long targ, int SPD, Wheels& wheels)
/* Performs one iteration of adjustment for wall following.
...
void forward_to_dist_wall(long stop_dist, long wall_dist, Wheels& wheels, Ultrasonic& front_US, Ultrasonic& side_US, int SPD)
// Wall follow until front sensor hits certain distance
{
  ...
  while (check_array(front_US.array, stop_dist) != 1)
  {
    ...
    if (loops % 2 == 0) wall_straight_adjust(side_US, SPD, wheels);
    else wall_dist_adjust(side_US, wall_dist, SPD, wheels);
    loops++;
  }
  wheels.Stop();
}
